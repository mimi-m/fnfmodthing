...
function onUpdatePost(elapsed)
    for a = 0, getProperty('notes.length') - 1 do
		...
		if not mustPress and noteType == 'Confuse Note' then
			if (strumTime - getSongPosition()) / getProperty('songSpeed') < 1 and getPropertyFromGroup('notes', a, 'active') then
...
function opponentNoteHit(id, dir, nt, sus)
    ...
    if getProperty('health') > 0.0500 then
        setProperty('health', health- 0.0195 * (healthGainMult / healthLossMult));
    end
--	setPropertyFromGroup('opponentStrums', dir, 'visible', true)
--	setProperty('fakeN'..dir..'.visible', false)
--	if noteType == 'Hurt Note (Miss Anim)' then
--		playSound("ANGRY");
--		setProperty('health', getProperty('health') + 0.175)
--	end
